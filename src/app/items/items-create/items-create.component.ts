...
export class ItemsCreateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.createItemForm.controls.itemImage.setValue(this.defaultImage);
  }
  onSubmitItem(err) {
    if (this.createItemForm.invalid) {
      return;
    }
    this.itemsService.createItem(this.createItemForm.value).subscribe(saveditems => {
      if (saveditems) {
  ...
  imageCropped(event: ImageCroppedEvent) {
    this.createItemForm.controls.itemImage.setValue(event.base64);
  }
  imageLoaded() {
    // show cropper
  }
